...
//                 char_count = 1;
//                 line_pcb = (char*)realloc(line_pcb, 2);
//                 memset(line_pcb, 0, 2); // errase data inside  
//                 line_pcb[1] = '\0';
...
//         } else {
//             char_count++;
//             line_pcb = (char*)realloc(line_pcb, char_count); // will not efeect the data in line_pcb
...
//     int char_count = 1;
//     int value_number = 0;
//     char* value = (char*)malloc(1); // change it to dynamic allocation
...
//             }
//             char_count++;
//             value = (char*)realloc(value, char_count + 1); // extra 1 is for null terminator
...
//                     value = (char*)realloc(value, 1); // realloc the value to its original
//                     char_count = 1;
//                     value[1] = '\0';
//                     break;
//                 case 1:
...
//                 exit(1);
//             }
//             memset(value, 0, char_count); // setting all the bytes in memory will make it equivalent to empty string

//             if (value_number > 6) {
//                 fprintf(stderr, "ERROR ON: parser function (csv file data unvalid)");
//                 free(parsed_line);
//                 exit(1);
//             }
//         } else {
...
//     // correcteda: Initialize all fields
...
//     // CHANGED: Use strlen to determine actual string length instead of fixed 60001
//     int value_len = strlen(value);
//     for (int i = 2; i <= value_len; i++) { // CHANGED: Loop until <= value_len to catch '\0'
//         // CHANGED: Check for end of string (missing ']')
//         if (value[i] == '\0') {
//             fprintf(stderr, "ERROR ON: instruction_parser missing ']' at end of instruction list %s\n", value);
//             free_instructions_chaine(returned->instructions_head);
//             free(returned);
//             exit(1);
//         }

//         if (value[i] != ',' && value[i] != ']' && instruction_char_count < 3) { // if value is a ressource character and we didnt arrive to the end which is 3characters
...
//             // correcteda: Proper chaining logic
...
//             // correcteda: Proper chaining logic for last node
//             if (returned->instructions_head == NULL) {
//                 returned->instructions_head = end;
//                 returned->instructions_fin = end;
//             } else {
...
//         } else if (i == value_len) { // CHANGED: Use actual string length
...
//     // correcteda: Check if we have at least one instruction when list is not empty
...
// //     // correcteda: Initialize struct fields to prevent garbage values
// //     returned->instructions_head = NULL;
// //     returned->instructions_fin = NULL;
// //     returned->count = 0;
...
// //         // correcteda: Check for end of string before accessing value[i]
// //         if (value[i] == '\0') {
// //             fprintf(stderr, "ERROR ON: instruction_parser missing ']' at end of instruction list %s \n", value);
// //             free_instructions_chaine(returned->instructions_head);
// //             free(returned);
...
// //         // correcteda: Fixed condition - removed "value[i] == '['" which should only be at position 0
// //         if (value[i] != ',' && value[i] != ']' && instruction_char_count < 3) { // if value is a ressource character and we didnt arrive to the end which is 3characters
...
// //             // correcteda: Fix logic - check count BEFORE incrementing
// //             if (returned->count == 0) {
// //                 returned->instructions_head = (INSTRUCTION*)malloc(sizeof(INSTRUCTION));
// //                 if (returned->instructions_head == NULL) {
// //                     fprintf(stderr, "ERROR ON: instruction_parser failed allocating the instruction\n");
// //                     free(returned);
// //                     exit(1);
// //                 }
// //                 returned->instructions_fin = returned->instructions_head;
// //                 returned->instructions_fin->next = NULL;
// //             } else {
...
// //                     exit(1);
// //                 }
// //                 returned->instructions_fin = returned->instructions_fin->next;
// //                 returned->instructions_fin->next = NULL;
// //             }
...
// //             // correcteda: Increment count AFTER node creation
// //             returned->count++;

// //             // add it to chaine
// //             INSTRUCTION* end = add_instruction_type(returned->count, returned->instructions_fin, instruction);   // correcteda: Use instructions_fin not instructions_fin->next
...
// //     // correcteda: Check if we got any instructions (for empty list case [])
// //     if (value[1] == ']' && returned->count == 0) {
// //         // Empty instruction list is valid
// //         return returned;
// //     }
